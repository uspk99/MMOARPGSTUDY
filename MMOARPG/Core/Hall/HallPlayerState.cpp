...
bool AHallPlayerState::IsCharacterExistInSlot(const int32 InPos)
{
	return CharacterAppearances.FindByPredicate(
		[InPos](const FMMOARPGCharacterAppearance &InCA)
		{
...
int32 AHallPlayerState::RemoveCharacterAppearanceBySlotID(const int32 InSlotID)
{
	//FMMOARPGCharacterAppearance* DeleteCA = GetCharacterCA(InSlotID);
	
	return CharacterAppearances.RemoveAllSwap([InSlotID](const FMMOARPGCharacterAppearance& InCA)
		{
			return InSlotID == InCA.SlotPosition;
		});
}
